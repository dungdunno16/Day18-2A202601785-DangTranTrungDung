...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    if OPENAI_API_KEY:
        try:
            ...
            extracted = [q.strip().lstrip("0123456789.-) ") for q in questions if q.strip()]
            if extracted:
                return extracted[:n_questions]
        except Exception as e:
    ...
    fallback_qs = []
    for s in raw_sentences[:n_questions]:
        clean_s = s.rstrip(".!?")
        fallback_qs.append(f"{clean_s}?")
    return fallback_qs
...
def _enrich_single_call(text: str, source: str) -> dict:
    ...
    if OPENAI_API_KEY and not _api_disabled:
        try:
            from openai import OpenAI
            client = OpenAI(base_url=OPENAI_BASE_URL or None)
            resp = client.chat.completions.create(
                    ...
                    {"role": "system", "content": """Phân tích đoạn văn và trả về JSON:
{
  ...
  "metadata": {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}
            ...
            if "429" in str(e) or "limit" in str(e).lower() or "quota" in str(e).lower():
                _api_disabled = True
                print("  💡 Đã vượt quá quota/rate limit API. Tự động chuyển sang chế độ làm giàu offline (extractive) nhanh cho các chunk còn lại.")

    # Pure extractive fallback (0 API calls, runs instantly offline)
    sentences = [s.strip() for s in text.replace("\n", " ").split(". ") if s.strip()]
    summary = ". ".join(sentences[:2]) + "." if sentences else text
    raw_sentences = [s.strip() for s in re.split(r'[.!?\n]', text) if len(s.strip()) > 10]
    fallback_qs = [f"{s.rstrip('.!?')}?" for s in raw_sentences[:3]] if raw_sentences else [f"{text[:50]}?"]
...
def enrich_chunks(
    ...
    for i, chunk in enumerate(chunks):
        ...
        merged_meta = {**auto_meta, **chunk.get("metadata", {})}
